...
- **Email connections (OAuth) stay single-user for everything except the From-identity list, and tokens are never account-visible.** Connecting/disconnecting a mailbox, and the two encrypted token columns, remain strictly `user_id = auth.uid()` — no CRO Leader bypass (Backend Schema §6.3). The one client-confirmed exception (2026-09-08): the non-secret identity columns (`provider`, `email_address`, `status`) are readable account-wide via a broadened SELECT policy plus a column-level `REVOKE` on the token columns, so the Contact Detail "quick send" From-picker can list a teammate's connected mailbox by name/address without ever exposing (or using) their tokens. Don't widen this further, and don't add a CRO Leader bypass to it.
- **Outbound email (quick-send and future Campaigns) goes through Resend regardless of which mailbox is connected** — the OAuth connection verifies/display an identity, it does not send mail via the provider's own API (Tech Stack Lockfile §5.2, amended 2026-09-08 from SendGrid/Nodemailer). Don't implement Gmail/Graph send APIs.
