...
| I5 | Settings | GOS Solution Questionnaire (client-confirmed addition, 2026-09-15; renamed from "GOS Solution Questionnaire" everywhere users see it, 2026-09-17 — sidebar label stays "GOS Questionnaire") | Owner, Admin edit; other MSP roles view; CRO Admin/Advisor and a granted partner get the same read+write parity they have elsewhere |
| I5b | Settings | GOS Vision Board (client-confirmed addition, 2026-09-16) — nav label "GOS Vision Board" (amended 2026-09-16, matching I5's "GOS Questionnaire" shortening), page heading "GOS Vision Board". Client-confirmed redesign (2026-09-17): once complete, the page shows every answer as a strategy document (sticky section list, one block per section in wizard order) with "Edit Vision Board" (editors only) and "Export PDF" (anyone who can view, GET /api/vision-board/export), replacing the earlier sign-off banner and "Coming soon" deliverables preview | Owner, Admin edit; other MSP roles view; CRO Admin/Advisor and a granted partner get the same read+write parity they have elsewhere |
...
**Client-confirmed addition (2026-09-15):** a full-width banner sits above the KPI strip while the account's Growth Questionnaire (§4.9, I5) is incomplete — "Complete your GOS Solution Questionnaire," a short progress line, and its own progress bar.

**Client-confirmed addition (2026-09-16):** a second, independent banner ("Complete your GOS Vision Board," I5b) sits directly beneath the Questionnaire one — stacks regardless of either's complete/incomplete state. The notification bell (Design System §8.10) gets the same treatment: a second dropdown item, independent dot condition, both can show at once.

**Client-confirmed amendment (2026-09-16):** both Dashboard banners now stay permanently visible instead of disappearing once complete — superseding the "disappears entirely" language above. Complete, each switches to a green state ("GOS Solution Questionnaire complete" / "GOS Vision Board complete," "X of X answered," a check icon, "View →" instead of "Continue →") rather than vanishing; nothing else on the Dashboard shifts to fill the space either way. The notification bell dropdown is unaffected by this — it still drops its item and clears its dot once a document is complete, since a finished item genuinely needs no further action from a notification center.
...
**Client-confirmed amendment (2026-09-17):** both Dashboard banners drop the "Complete your…" / "…complete" wording and the extra in-progress sentence — in every state the title is just the document's name ("GOS Solution Questionnaire" / "GOS Vision Board"), the line is just the answered count (e.g. "75 of 75 questions answered"), and the button always reads "View →". Green when complete, blue while in progress, as before.
...
- **Growth Questionnaire (I5, client-confirmed addition 2026-09-15)** — Account Settings, right after Company. The 8-section, 75-question GOS Solution Questionnaire (source: "Growth Solution Questionnaire for MSPs.docx"), presented one section per screen ("Concept A — Stepped," approved from two compared mockups) with a dot-and-line progress stepper, Back / Save & Continue, and "Skip for now, remind me later" always available — nothing here is required to proceed. Each question's input matches what it's actually asking (a number field for a count, Yes/No for a yes/no question, a Hunters/Farmers choice, the one 1–4 scale question, free text for the rest), not a blanket text box. Answers save to `growth_questionnaire_responses` on every Continue/Skip/Finish. Once every question has an answer, "Export PDF" (§10, GET /api/questionnaire/export) becomes available in the wizard itself.
  ...
  - **Surfaced while incomplete, in two places, both client-confirmed:** a full-width Dashboard banner ("Complete your GOS Solution Questionnaire," its own progress bar, disappears entirely once done — §4.3) and the top bar's notification bell, which is otherwise fully decorative (Design System §8.10 — it was deliberately left with no fake unread state until a real notification existed). This is that first real notification: a red dot only while incomplete, one item linking to the questionnaire, an honest "You're all caught up" empty state once it's done.
...
**Client-confirmed rename (2026-09-17):** the two documents are named **GOS Solution Questionnaire** and **GOS Vision Board** everywhere users see them — page headings, browser titles, Dashboard banners, the notification bell, cross-references, and the exported PDFs (including their file names). The Settings nav keeps its shorter labels, "GOS Questionnaire" and "Vision Board."
