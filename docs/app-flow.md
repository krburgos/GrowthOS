...
**Client-confirmed Command Center header changes (2026-09-24).** Three things:

1. **The "Before You Begin" strip moves above the hero band**, so the first thing on the page is what is blocking it.
2. **It measures the three account documents** - Company Profile, Solution Questionnaire, Vision Board - rather than the source doc's original website-and-ICP pair, warning while any is incomplete and linking to whichever one is missing. This is a **stricter** test, not a looser one: the website is one of the Company Profile's eleven completeness fields and the written ICP is one of the Questionnaire's questions, so both original foundations are still required, as part of finishing the documents that contain them. The hide/show toggle is kept, with a versioned storage key so anyone who dismissed the old strip sees the new one once rather than inheriting a dismissal of a different question.
3. **The hero's descriptive sentence is replaced by a badge** reading "Powered by AI and CRO Leader" (approved treatment "C": a translucent pill in the same material as the quarter pill opposite it). That sentence was also the only thing telling a reader the cards are clickable, so that hint moves to the Mission Cards heading, where someone about to click one is looking.
...
**Client-confirmed rework (2026-09-25) — the Mission Card opens onto a task list.** The client's framing: GrowthOS is more than a CRM, so after reading a workstream's status report the MSP should see what to do about it and who is doing it. The 14 Mission Cards stay exactly where they are, on the Command Center grid — clicking one is still how you get to the workstream, and the work itself lives on that step's detail page (C3), not on the card.

- **On the card**, the assignee avatar stack added on 2026-09-24 gives way to a task summary in the same slot: "X of Y done" with a thin progress bar, the avatar stack beneath it, and — on the right — an amber "N unassigned" chip when anything open has no owner, or the remaining hours of work when everything is assigned. A workstream with no tasks reads "No tasks yet". The card stays a summary (client-confirmed); it answers "is anything happening here, and who has it", and nothing more.
- **On the step detail page**, a **"What to do next"** panel sits between the Hours panel and the SEO/GEO tabs, and appears on all 14 workstreams rather than only the two with the three-tab shape. Each task shows its title, detail, priority, hours, due date and assignee. **It replaces the Suggestions & Fixes tab** (client-confirmed) — that list was already CRO-authored "here is what to fix" with a priority and a detail line, which is a task without an owner, hours, a state or a due date; the existing suggestions were carried across as tasks. SEO and GEO therefore now show two tabs, Status Report and Progress Tracker.
- **Who does what.** Only CRO Leader (CRO Admin/Advisor) creates, edits or archives a task — they prescribe the work. The account's MSP Owner and Admin can change a task's state and who it is assigned to, inline, without opening a dialog. Everyone else is read-only. Marking work done is not the same as prescribing it, and completing a task moves achieved hours, which MSP Owner/Admin already control. A database trigger enforces that same split (Backend Schema §6.6f); the UI only mirrors it.
- **Assignees** come from either Company Profile roster — Sales & Marketing or Outsourced (§4.8, Backend Schema §6.6e) — so the same people who carry weekly hours commitments are the people tasks land on. Tasks are **not** tied to a quarter, but completing one adds its hours to that workstream's achieved hours for the quarter it was completed in, and re-opening it takes them back out.
- **Task states** are Active, In progress, On hold and Complete, set manually — nothing infers a state from hours logged or due dates passing.

**Client-confirmed restructure (2026-09-25, second pass) — the workstream page loses its tabs.** Asked to rework the page around the report, the client settled five things: Status Report goes on top, the Progress Tracker tab is no longer needed, the tasks sit directly below the report, the board should read like a Monday.com board, and all fourteen workstreams get a Status Report rather than only SEO and GEO. The page is now a single scroll with no tab strip at all — with the tracker gone and the report promoted, there was nothing left to tab between:

1. **Hours** (unchanged).
2. **Status Report** — the summary paragraph and its figures, on all 14 steps. CRO Leader writes both; everyone else reads. Content is typed by hand (client-confirmed); pulling figures from an analytics tool was discussed and deliberately deferred.
3. **What to do next** — the task board.
4. **Duties**, then **KPIs** (unchanged).

**The board (client-confirmed, grouped rows over kanban).** Tasks are grouped by priority — High, Medium, Low — each group carrying a coloured left bar, its task count and its total hours, and each row showing Task, Owner, Status, Due and Hrs as real columns. The status cell fills its whole width in colour, which is what makes a Monday board read as one at a glance. State stays a *column* rather than becoming the grouping, which is the substantive difference from a kanban: hours, owner and due date all need to be readable without opening anything, and a kanban card hides them. The board scrolls sideways on a narrow screen rather than dropping columns — an owner or a due date you cannot see is the reason a task gets missed. Owner, Status, Hrs and Due are all changed inline by MSP Owner/Admin (client-confirmed, 2026-09-25); the pencil beside the hours opens the CRO-only editor for everything else. Hours and the due date are click-to-edit, committing on Enter or on leaving the field and abandoning on Escape, and clearing the date removes it — a real state, since due dates are wanted but not required. Those four fields are how the work is *tracked*, which is the account's to say, while the title, detail, priority and workstream are what the work *is* and stay with CRO Leader. **Status is a colour palette (client-confirmed, 2026-09-25):** one click opens four coloured options, one more sets it. It replaced a Select, which was a form control doing a board cell's job. Both inline changes are applied optimistically - the row moves the moment it is clicked and rolls back with a toast if the write fails - because the previous behaviour waited for the write and then re-rendered the whole server page, which put a visible pause between the click and the colour changing. The refresh still runs after a successful write, since completing a task moves the workstream's achieved hours through a database trigger and the Hours panel above has to catch up. A due date that has passed on unfinished work turns red.

**Export (client-confirmed) — the Status Report downloads as a PDF**, via `GET /api/gos-dashboard/[slug]/export`. It carries the report, its figures, the quarter's hours and the task board, so the exported document answers both "where does this stand" and "what happens next" — a report without the tasks would be half the page. PDF rather than the Reports spreadsheet because this is a document someone reads, not a dataset someone filters; it follows the Vision Board and Questionnaire exports, Poppins throughout. The route reads through the same functions the page uses, so the PDF can never show something the reader is not allowed to see on screen.
...
**Client-reported gap, fixed (2026-09-25) — the roster and the Mission Cards were reading only half of "who works on this workstream."** There are two notions of it and they are deliberately separate: a **declared** assignment is a row in `account_team_assignments`, set in the Company Profile, which is a plan and the only thing that can carry planned weekly hours for a workstream; **from tasks** is simply holding tasks in that workstream on the Command Center board. Only the declared side was being read, so on an account where nobody had filled in Areas of Responsibility, four people holding sixty-seven tasks across ten workstreams each showed as doing nothing — "None yet" on the roster, and "Nobody assigned yet" on all fourteen Mission Cards.

Both are now shown, and never silently reconciled. In Areas of Responsibility a declared assignment keeps the solid cyan chip with its weekly hours; a workstream known only from tasks is a dashed outline chip carrying its open-task count, and says which it is on hover. On a Mission Card the avatar stack now includes people who merely hold tasks there, with the same distinction in the tooltip. `membersForStep` returns `declared` alongside `hours` so a caller can tell them apart — and `hours` is 0 for someone known only from tasks, so it must not be summed and presented as everybody's commitment.
...
**Client-confirmed KPI band revision (2026-09-22, amended 2026-09-24):** the two groups read **Prospects** and **Opportunities**, and the halves now split by what they count rather than by how the source doc drew them — Prospects holds the boxes fed by contact statuses, Opportunities the five fed by opportunity stages. Engaged moved into Prospects on 2026-09-22 and was **removed entirely on 2026-09-24** along with the contact status behind it, so Prospects is MQCs and MQLs. No box was added or removed, so saved mappings carry over untouched. The per-box line naming the source statuses/stages is gone as redundant with the box label, the figures are a type step larger, and they animate up from zero on load. The Opportunities header gained the **Active** pill described above.
