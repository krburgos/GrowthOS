...
| F2 | Foundation | GrowthOS Solution Questionnaire (client-confirmed addition, 2026-09-15; renamed from "Growth Solution Questionnaire" everywhere users see it, 2026-09-17 — sidebar label stays "GOS Questionnaire") | Owner, Admin edit; other MSP roles view; CRO Admin/Advisor and a granted partner get the same read+write parity they have elsewhere | **Moved out of Settings to the Strategy section, 2026-09-24 (§4.12); `/strategy/solution-questionnaire`, old path redirects.**
| F3 | Foundation | GrowthOS Vision Board (client-confirmed addition, 2026-09-16) — nav label "Vision Board", page heading "GrowthOS Vision Board". **Client-confirmed redesign (2026-09-22): this route is now the wizard only.** A completed board lives at `/vision-board` (§4.11), its own full-width page, and landing here with one already finished redirects there unless `?edit=1` says the visitor pressed Edit. Supersedes the 2026-09-17 strategy-document view, which itself replaced the sign-off banner and "Coming soon" grid | Owner, Admin edit; other MSP roles view; CRO Admin/Advisor and a granted partner get the same read+write parity they have elsewhere | **Moved out of Settings to the Strategy section, 2026-09-24 (§4.12); `/strategy/vision-board`, which now serves both the finished page and the wizard. Old `/settings/vision-board` and `/vision-board` paths redirect.**
...
### 4.12 Foundation (`/foundation`)

**Client-confirmed (2026-09-24, approved mockups).** A top-level sidebar section holding the three documents an account must complete before the Command Center will run: **Company Profile** (`/foundation/company-profile`), **Solution Questionnaire** (`/foundation/solution-questionnaire`) and **Vision Board** (`/foundation/vision-board`). The index lists all three as cards showing done-out-of-total, a meter, a status pill and Start / Continue / View — the same card language the Dashboard's setup block uses, on white rather than navy.

**Sequence and sub-items.** The rail runs Homepage, **Foundation**, Command Center, Contacts, Companies, Opportunities, Lists, Campaigns, Reports, Settings (client-confirmed sequence "A"). Foundation sits directly above Command Center because it feeds it — the readiness strip blocks that page until all three are done — so read top to bottom the rail tells the story: your day, what you set up, the plan being executed, the data, the reporting, the plumbing. It is the first sidebar item with **children**: expanded, each document shows a dot (green complete, amber outstanding); collapsed, the children hide and the icon carries a count badge of what is left, which disappears at zero.

**The name.** "Foundation" over "Strategy" (the section's name for a few hours on 2026-09-24) once Company Profile joined it: the Command Center's readiness strip already says **"all three foundations"**, so the sidebar and that page now use the same word for the same three things.

**Why they left Settings.** Settings is where you change configuration; these are written once, read often, exported as PDFs and consumed by other screens — the Command Center's readiness check reads the Questionnaire's ICP answer, and a finished Vision Board is a page you would show a board. The Vision Board had already half-escaped, with its finished view at a top-level route and its editor three levels into Settings; this finished a move already underway.

**Company Profile moved here too (2026-09-24), reversing the earlier decision to leave it in Settings.** The argument that changed it: if the product calls these three *foundations* and blocks the Command Center until all three are done, then burying one of them in Settings while the other two sit in the sidebar is the inconsistency. It leaves Settings entirely rather than appearing in both places — two doors to one page is worse than either door alone — so Account Settings now holds Billing, Email Auth, Users, Integrations and Customizations, which is a coherent list of account administration. `/settings/company` redirects, and Settings' own "Account Settings" entry point now lands on Users & Roles.

**Also considered:** Questionnaires (names the input form, and the Vision Board stops being one the moment it is finished), Blueprint, Mission Brief, and Strategy — which was used briefly and then replaced when Company Profile joined, since it held only two documents at the time.

**Routing.** `/foundation/vision-board` serves both surfaces: a completed board renders the Vision Page, `?edit=1` renders the wizard, and an unfinished board renders the wizard regardless — one address per document, and one redirect hop fewer than before. The old paths (`/settings/company`, `/settings/growth-questionnaire`, `/settings/vision-board`, `/vision-board`, and the short-lived `/strategy*`) redirect rather than 404, each straight to its final destination rather than hopping through the rename, because the Dashboard setup block, the readiness strip, the notification bell and the invite flow all linked to them, as may anyone's bookmarks. Both rows are removed from the Settings nav in the same change, so there are never two doors to the same room.
