...
- **Emails** — email history specifically (a filtered view of Activity, for quick scanning of just correspondence). **Client-confirmed addition (2026-09-08):** each entry expands to show the full From/To (the sender's name/email and this contact's name/email — both already implicit in the underlying Activity row, no new fields) and the complete message body, not just a snippet.

**Client-confirmed addition (2026-09-08):** an "Email" button (opens a compose dialog: Subject + Message, To fixed to this contact) and, when a phone number is on file, a "Call" button (a plain `tel:` link — no telephony integration in Phase 1) sit above the stat row on Contact Detail's main content, next to Open Pipeline. Sending an email goes out through the same SendGrid relay Milestone 10's Campaigns will use (not the recipient's connected mailbox) and automatically logs an Activity — no separate "log this" step.
