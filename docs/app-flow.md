...
| S1 | Strategy | GrowthOS Solution Questionnaire (client-confirmed addition, 2026-09-15; renamed from "Growth Solution Questionnaire" everywhere users see it, 2026-09-17 — sidebar label stays "GOS Questionnaire") | Owner, Admin edit; other MSP roles view; CRO Admin/Advisor and a granted partner get the same read+write parity they have elsewhere | **Moved out of Settings to the Strategy section, 2026-09-24 (§4.12); `/strategy/solution-questionnaire`, old path redirects.**
| S2 | Strategy | GrowthOS Vision Board (client-confirmed addition, 2026-09-16) — nav label "Vision Board", page heading "GrowthOS Vision Board". **Client-confirmed redesign (2026-09-22): this route is now the wizard only.** A completed board lives at `/vision-board` (§4.11), its own full-width page, and landing here with one already finished redirects there unless `?edit=1` says the visitor pressed Edit. Supersedes the 2026-09-17 strategy-document view, which itself replaced the sign-off banner and "Coming soon" grid | Owner, Admin edit; other MSP roles view; CRO Admin/Advisor and a granted partner get the same read+write parity they have elsewhere | **Moved out of Settings to the Strategy section, 2026-09-24 (§4.12); `/strategy/vision-board`, which now serves both the finished page and the wizard. Old `/settings/vision-board` and `/vision-board` paths redirect.**
...
### 4.12 Strategy (`/strategy`)

**Client-confirmed (2026-09-24, approved mockup).** A top-level sidebar section between Reports and Settings, holding the two documents the rest of GrowthOS reads from: **Solution Questionnaire** (`/strategy/solution-questionnaire`) and **Vision Board** (`/strategy/vision-board`). The index page lists both as cards showing answered-out-of-total, a meter, a status pill and Start / Continue / View — the same card language the Dashboard's setup block uses, on white rather than navy.
...
**Company Profile deliberately stayed in Settings.** It is the account's own record — name, address, logo, phone, CEO, team — changed when the company changes, and it belongs beside Billing, Email Auth and Users. It only ever sat with the other two because all three appear together on the Dashboard as onboarding steps, which is a statement about onboarding rather than about what kind of thing each one is.

**Naming.** "Strategy" was chosen over Questionnaires (names the input form, and the Vision Board stops being one the moment it is finished), Foundations (the readiness strip's own word, but that strip counts three including Company Profile), Blueprint and Mission Brief. It also echoes the Command Center's subtitle, "Strategy & Assignments", so the two read as related.

**Routing.** `/strategy/vision-board` serves both surfaces: a completed board renders the Vision Page, `?edit=1` renders the wizard, and an unfinished board renders the wizard regardless — one address per document, and one redirect hop fewer than before. The three old paths (`/settings/growth-questionnaire`, `/settings/vision-board`, `/vision-board`) redirect rather than 404, because the Dashboard setup block, the readiness strip, the notification bell and the invite flow all linked to them, as may anyone's bookmarks. Both rows are removed from the Settings nav in the same change, so there are never two doors to the same room.
