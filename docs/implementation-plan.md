...
**Client-confirmed addition, out of sequence (2026-09-08):** a single-contact "Email" button was added to Contact Detail (Milestone 6/8's screen) ahead of Milestone 10, sending one message at a time via `POST /api/email/send`. The docs were silent on this narrower case — only bulk Campaign sending was speced. Rather than wait for Milestone 10, the client asked for it now and gave an explicit build direction: use the same SendGrid SMTP relay via Nodemailer that Milestone 10 will also use (Tech Stack Lockfile §5.2's "regardless of which mailbox is connected" rule was flagged to the client as applying here too, since it reads as a blanket rule and not one scoped to bulk sends only — they confirmed the relay approach rather than sending via the recipient's connected Gmail/M365 mailbox). The `email_connections` row from Milestone 9 still isn't used for sending; the logged-in sender's real address is only set as the Reply-To header so replies still reach them. Every send is logged as an `activities` row (`type = 'email'`) automatically — no new columns needed, since `user_id`/`contact_id` already capture From/To and the Emails tab (`components/activities/email-list.tsx`) now expands each entry to show both explicitly, pulled from `.env.example`'s existing `SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASSWORD`/`EMAIL_FROM_ADDRESS` vars (declared but previously unset/unused). Milestone 10 below is unaffected in scope — it's still the bulk/list-targeted Campaigns pipeline, unlocked by this same relay config.
