...
Claude Code cannot create third-party accounts, register OAuth applications, or edit DNS — these need a person with access to the client's Google/Microsoft/GitHub/Vercel/Supabase/Resend accounts and the sending domain. The table below is everything that needs a human, in the order it's actually needed (not all of it is needed on day one).
...
| 7 | Resend account, sending domain authenticated (SPF/DKIM/DMARC records on the client's DNS) | Milestone 10 (already needed by the quick-send feature, done 2026-09-08) | Client — DNS access on the domain email will send from |
| 8 | Resend webhook pointed at /api/webhooks/resend | Milestone 10 | Client, from the Resend dashboard — can point at the Vercel preview URL for early testing, then be updated to the production domain at go-live |
...
- Human verification: actually receive and click one real invite email end to end (Supabase's default email delivery is fine for this — Resend isn't wired up until Milestone 10), to confirm the invite → set password → login path works outside of a seeded/simulated session.
...
- Build the companies list/detail screens, including the merge-companies flow (Backend Schema §7.3's merge_companies function) as a deliberate user action, not automatic. **Resolved 2026-09-08** (later than the rest of this milestone): the list/detail screens weren't actually built at the time — merge lived inline on Contact Detail's Company card instead, the only place available. App Flow §4.4 (D5–D6) now covers the real Companies List/Detail screens this line originally asked for.
...
- Build GET /api/oauth/[provider]/start and GET /api/oauth/[provider]/callback (Backend Schema §10). Request the minimal scope only — email address and basic profile (e.g., Google's userinfo.email, Microsoft's User.Read) — since actual sending goes through Resend (amended 2026-09-08 from a SendGrid relay) in Milestone 10, not through the provider's own send API; this connection exists to verify and display a legitimate from-address, not to grant mail-send permission.
...
**Client-confirmed addition, out of sequence (2026-09-08):** a single-contact "Email" button was added to Contact Detail (Milestone 6/8's screen) ahead of Milestone 10, sending one message at a time via `POST /api/email/send`. The docs were silent on this narrower case — only bulk Campaign sending was speced. Rather than wait for Milestone 10, the client asked for it now and gave an explicit build direction: use a relay rather than sending via the recipient's connected Gmail/M365 mailbox (Tech Stack Lockfile §5.2's "regardless of which mailbox is connected" rule was flagged to the client as applying here too, since it reads as a blanket rule and not one scoped to bulk sends only). Every send is logged as an `activities` row (`type = 'email'`) automatically.

**Client-confirmed follow-up, same day (2026-09-08):** the quick-send feature above grew a **From identity picker** and a **Cc field**, evaluated with the client through several rounds of explicit questions rather than assumed:
- **Vendor switch, SendGrid → Resend:** while comparing options for "least configuration, cost, best deliverability," Resend was recommended over SendGrid/Postmark/SES/real-Gmail-Graph-sending and the client confirmed switching the *entire* project's outbound email vendor, not just this feature — see Tech Stack Lockfile §5.2 for the full comparison. Nodemailer is removed entirely (no longer needed — Resend's SDK talks to its own HTTPS API, not SMTP). Milestone 10 below is updated to match; its scope is otherwise unaffected — still the bulk/list-targeted Campaigns pipeline, unlocked by the same Resend config this feature now depends on.
- **From, account-wide (client-confirmed reversal of part of Backend Schema §6.3):** the client explicitly asked for the From list to show every connected mailbox in the account, not just the sender's own — reopening the "strictly single-user, no exceptions" decision from Milestone 9. Confirmed only after the client understood the real mechanism: this **stays relay-based** (Resend), so picking a teammate's identity only changes the outgoing email's display From name and Reply-To — it does not send through that teammate's Gmail/Outlook, and they aren't notified or asked. Implemented as a genuinely narrower reversal than a blanket RLS opening, to avoid also exposing tokens account-wide: `email_connections`' SELECT policy now allows any account member to see rows for other users in the same account, but the two token columns (`access_token_encrypted`, `refresh_token_encrypted`) are separately `REVOKE`d at the column-privilege level from the `authenticated` role, so only `provider`/`email_address`/`status`/etc. become visible — tokens stay exactly as private as before, and INSERT/UPDATE (connecting/disconnecting your own mailbox) are untouched. See Backend Schema §6.3 and §5.5 for the exact policy/columns and the new `activities.send_from_connection_id`/`activities.cc` columns this required.
- **Cc:** a plain comma-separated free-text field, client-confirmed as not needing to match existing GrowthOS contacts.

## 13. Milestone 10 — Campaigns, Scheduled Sending & Tracking

**Goal:** the full send pipeline — compose, schedule, send via Resend, and record opens/clicks/bounces/unsubscribes — exactly as sequenced in Backend Schema §8–§9.

**Built 2026-09-08.** Discovered mid-build that the entire database layer — the campaigns/campaign_recipients/campaign_events tables, all their RLS policies, record_campaign_event()/send_due_campaigns(), the vault.secrets app_url/cron_secret, and the every-minute `send-due-campaigns` cron.job — was already live from an earlier migration pass (Milestone 2's comprehensive schema setup), even though none of the application code existed yet. So this milestone ended up being purely the app layer:
- Campaign composer (`/campaigns/new`) — plain-text/basic formatting only, no rich-text editor (Tech Stack Lockfile §5.3), targeting a list (Milestone 7), From defaulting to the composer's own identity or any account-wide connected mailbox (the same picker/pattern as the Contact Detail quick-send feature above). Saves as a `draft` row; sending is a separate, reviewable step from Campaign Detail.
- POST /api/campaigns/[id]/send — "Send Now" and "Schedule for Later" are the same transition (status → 'scheduled', scheduled_at now or future); there's deliberately no separate immediate-send code path, so the already-running cron is the only thing that ever actually sends.
- POST /api/campaigns/send-due — resolves list members (excluding `email_opt_out`), creates any missing `campaign_recipients` rows, and sends each recipient their own email via `lib/email/resend-send.ts` (the same real-address-with-fallback helper the quick-send route uses, extracted into a shared lib during this milestone) with a personalized tracking token baked into the body.
- `lib/email/build-campaign-html.ts` — converts the plain-text body into simple HTML: bare URLs rewritten to route through the click-tracking redirect, an unsubscribe footer, and a 1×1 open-tracking pixel. The compose *experience* stays plain-text; only the *delivered* email is HTML, since a pixel/tracked-link can't exist in a plain-text send.
- GET /api/track/open/[token].gif, GET /api/track/click/[token], GET /api/unsubscribe/[token] — all three call record_campaign_event(), no session.
- POST /api/webhooks/resend — bounced/complained/delivered only (opens/clicks come from this app's own routes above, not Resend's native tracking, since the schema is already built around a per-recipient `tracking_token`, not a Resend message ID). Verified via a **hand-rolled Svix HMAC check** (`lib/email/verify-svix-signature.ts`, Node's built-in `crypto`) rather than adding the `svix` npm package — no new Tech Stack Lockfile dependency needed for one HMAC comparison. The recipient is identified by a `tracking_token` **tag** set on every Resend send call and echoed back in the webhook payload, not by matching on to-address.
- New env var: `APP_URL` (`.env.example`, Tech Stack Lockfile §7) — the app's own base URL, used to build the tracking/unsubscribe links above. Must be kept in sync with the `app_url` Vault secret Postgres already uses for the same purpose; both currently point at the production Vercel URL.
- Campaign Detail (`/campaigns/[id]`) — stat row (recipients/delivered/opened/clicked/bounced/unsubscribed) and a filterable per-recipient table, plus Send/Schedule controls when the campaign is still a draft. Campaigns Index (`/campaigns`) — the client-confirmed "Data Table" concept (approved mockup), a KPI strip, and the same table language every other list screen uses.

**Checkpoint:**
- Human verification required — send a real test campaign to a small, controlled list of real inboxes you control (2–3 addresses across providers is enough). Confirm: the email arrives and matches the composed content; opening it records an 'opened' event; clicking a link both redirects correctly and records a 'clicked' event; using the unsubscribe link records 'unsubscribed' and the contact's email_opt_out flips to true; a deliberately-bad test address produces a 'bounced' event via the Resend webhook (needs RESEND_WEBHOOK_SECRET set and the webhook URL configured in Resend's dashboard, neither of which Claude Code can do).
...
- **Done (2026-09-08):** the Reports screen — client-confirmed "Grid Dashboard" concept (approved mockup, one of five compared): all four App Flow §4.8 sections (Leads/Contacts Added, Opportunities by Stage, Campaign Performance, Revenue from Closed-Won) in a compact 2×2 grid, each with its own Export to Spreadsheet button per the client's explicit direction (not one export for the whole page). On-screen numbers are direct Supabase aggregate reads (no server route); GET /api/reports/export (ExcelJS) is the one route this screen needs, re-deriving the same aggregates per section on demand rather than sharing client state. Revenue sums straight from opportunities.value, never self-reported (PRD §6.7). The CRO Leader admin dashboard below is still not built.
- **Done (2026-09-08):** the CRO Leader admin dashboard (`/cro`) — search/list every MSP account (cross-account, unconditional for cro_admin/cro_advisor/cro_service_team) and "enter" one, and (CRO Admin only) create a new MSP account via POST /api/accounts (new accounts row + initial Owner invite in one call).
- **Client-confirmed addition, same day — the "partner" role and Partner Dashboard:** while scoping this milestone, the client asked for a *different* kind of cross-account access — a vendor/partner relationship that sees only specific MSP accounts it's been explicitly granted, not every account on the platform (unlike CRO Leader, which is all-or-nothing). This isn't in any of the six documents; see Backend Schema §5.2/§6/§12 for the full flag-and-confirm trail. Summary: a new `partner` user_role (no home account_id, same as CRO Leader), a `partner_account_grants` table (partner_user_id, account_id) that CRO Admin manages unilaterally (client-confirmed: no MSP-side consent step), and a matching `is_partner_for(account_id)` clause added to every tenant table's RLS alongside the existing `is_cro_leader()` checks. `/cro` doubles as the Partner Dashboard — same "search and enter an account" screen, just role-aware about which accounts show up — rather than being a separate screen, since the two are structurally identical once you have the RLS layer to back it.
- **The "viewing as" flow itself is also new as of this pass:** nothing previously existed to let a CRO Leader/partner session actually enter an MSP account — `getCurrentUser()` (`lib/auth/get-current-user.ts`) now resolves an effective `account_id` from a `growthos_viewing_account_id` cookie (set by POST /api/cro/enter, cleared by POST /api/cro/exit, which the CroLeaderBanner's "Exit to My Dashboard" now actually calls instead of just linking to /cro) — every existing MSP-shell page already reads `user.account_id` directly, so this was the one central change needed rather than touching each page. The cookie is a UI convenience only; RLS (`is_cro_leader()`/`is_partner_for()`) is what actually authorizes each query regardless of what the cookie says. Caught and fixed a related bug in the same pass: the Dashboard page redirected *any* CRO Leader role to /cro unconditionally, which would have broken "viewing as" the moment it existed — fixed to only redirect when no account has been entered yet.
**Checkpoint:**
...
- Update the Resend webhook URL (Prereq #8) to the production domain, and update both OAuth apps' (Prereq #5–6) redirect URIs from the preview URL to the production URL.
...
- **Access handoff**: confirm the client (not just the individuals who set up the accounts during this build) has admin access to Vercel, Supabase, Resend, and both OAuth app registrations — this build depended on five separate third-party accounts, and losing access to any one of them post-launch stalls anything from a routine key rotation to an incident response.
...
- **Milestone 9's OAuth scope (email/profile only, not mail-send)** follows directly from the Tech Stack Lockfile's decision to route all sending through Resend regardless of which mailbox is "connected" — this document didn't introduce that scope choice, just made explicit what it implies for the OAuth consent screens the client needs to configure.
