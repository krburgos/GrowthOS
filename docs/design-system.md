...
**Client-confirmed redesign, round two (approved mockup, "Concept B — Compact Header, Sectioned Card"), supersedes the original tall-banner version:** the original 96px gradient banner with a 128px overlapping avatar was felt to spend too much of a screen visited for seconds on decoration. `ProfileHeader` replaces `ProfileBanner`: a slim single-row navy strip (`primary-900` → `primary-800` gradient, no top-corner-only radius trick — the whole bar is `rounded-lg`) with a 56px avatar/logo (white/10 ring on the navy fill, not a white border on a light banner), the record's name in white/H4, an optional subtitle line (job title for My Profile, city/state for Company Profile) in white at 60% opacity, and an optional role/type pill (My Profile only — Company Profile has no equivalent concept) right-aligned. `ImageUploadCircle` gained an opt-in `compact` prop for this context — the camera button and its icon shrink (from `size-9`/`size-5` to `size-5`/`size-3`) to stay proportional on the smaller circle, since the original button was sized for 96–128px circles and would swallow a 56px one otherwise.
