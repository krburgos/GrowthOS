...
**Client-confirmed (2026-09-15) — that real source now exists.** The bell opens a `DropdownMenu` (same primitive as the avatar menu): while the account's Growth Questionnaire (§8.15) is incomplete, an error-600 dot sits on the bell and the dropdown holds one item — icon, "Complete your GOS Solution Questionnaire," an "X of 75 answered" caption — linking straight to the questionnaire. Once complete, the dot is gone and the dropdown reads "You're all caught up." rather than being empty with no explanation. Still exactly one honest state at a time, same rule the P2 fix established — no notification, no dot, ever.
...
### 8.15 GOS Solution Questionnaire
