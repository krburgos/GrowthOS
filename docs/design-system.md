...
**Client-confirmed reuse — Company Detail (2026-09-08):** the same single-column icon-label-value card (alternating `primary-700`/`secondary-600` icon badges, view/edit toggle, "Update Info"/Save/Cancel) reused verbatim for Company Detail's own profile card — not a Settings screen, but the identical pattern already carried the exact field set needed (it's the same fields as Contact Detail's Company card, App Flow §4.4). "Merge with another company" sits in the card header next to Save/Cancel while editing, moved here from Contact Detail's Company card (§8.5).
