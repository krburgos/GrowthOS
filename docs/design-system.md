...
- **Identity header** (new, above the tabs): a 64px circular avatar (`ContactAvatarUpload`, same `ImageUploadCircle` behind the Company Logo/Profile Avatar uploads — a client-confirmed **third** exception to the "no file attachments in Phase 1" line, `contacts.avatar_url` + a `contact-avatars` bucket) on a primary-500→secondary-500 gradient fallback, contact name (H1), a title/company subtitle line, and status/temperature/score chips — surfacing at-a-glance context that previously required opening the Overview tab.
- **Overview** now uses the same view/edit toggle as the Company Profile "Profile-style content card" (§8.9) instead of an always-editable form — icon-label-value rows grouped into a Contact card and a Company card (two columns on desktop), with an Edit button revealing inputs plus Save/Cancel. This closed an Impeccable-critique-flagged User-Control-and-Freedom gap (no Cancel, no view/edit signal) using the pattern already established elsewhere rather than inventing a new one.
