...
**Settings navigation panel** — a second docked column, rendered only while inside `/settings/*`, sitting between the icon rail and the page content (replacing the old horizontal tab bar that lived inside each Settings page). White background, 240px wide, 1px neutral-200 right border. Header: "Settings" in H4/primary-900. Each item is a full-width row, radius-md, 44px height: inactive state neutral-700 text on transparent, hover neutral-50 background; active state secondary-50 background with primary-700 text (same "selected" pairing already used for table rows, §8.5), no separate icon per item — this panel is a label list, not an icon list. Every Settings sub-page (Company Profile, Users & Roles, Connected Email Accounts, Contact Statuses, Opportunity Stages, My Profile) is a single, non-nested destination here — GrowthOS's Settings has no further sub-tab layer per page, unlike the reference CRM's deeper nesting, since none of GrowthOS's settings screens are that complex.
