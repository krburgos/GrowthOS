...
**Client-confirmed addition — Company LinkedIn:** the rail's icon-link row grew from two icons to three (Company Website, Company LinkedIn, the contact's own LinkedIn). The two company-associated icons share the neutral-100/neutral-500 treatment; the contact's personal LinkedIn keeps its existing secondary-50/secondary-700 treatment — so the color groups the two "about the company" links together and sets the "about this person" link apart, with the `title`/`aria-label` tooltip (`"[Company]'s LinkedIn"` vs. `"[Contact]'s LinkedIn"`) as the explicit disambiguator. Same field, same Link2 icon, and the same neutral-100 treatment were reused on the Contacts table's new Company LinkedIn column (§8.5) for consistency between the two surfaces.
