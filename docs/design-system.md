...
Background primary-900, item text white at 70% opacity (default), 100% opacity + primary-800 background pill (radius-md, inset 8px) when active/current page, 100% opacity with no background on hover. Disabled items (§2.4 of the App Flow Document): text at 30% opacity, no hover treatment, cursor: not-allowed. Icons (lucide-react, §8.13) sit left of the label, 20px, same opacity rules as the label. Collapsed (icon-only) state: icons remain, labels hidden, tooltip on hover showing the label.
