...
**Client-confirmed, Settings screen only, this exact stage only (2026-09-06):** the badge for the literal "Lost" stage in Settings → Opportunity Stages reads light-error red (Badge `error` variant) instead of the neutral-grey above — the client wants "Lost" specifically to visually read as a dead end on the management screen. This is keyed by stage **name**, not by Lost group membership: "Lost Resurrected" shares the same Lost group but stays neutral-grey, since the client confirmed only the literal "Lost" stage turns red, not everything the board treats as closed. The Kanban board's Lost/Stalled column headers in the table above are unchanged and deliberately stay neutral either way. `STAGE_GROUP_BADGE_VARIANT` (shared with the Opportunity List table and Contact Detail's opportunity rail) is untouched; the settings screen applies a local, name-keyed override on top of it.
