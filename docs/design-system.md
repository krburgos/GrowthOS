...
**Client-confirmed, Settings screen only (2026-09-06):** the Group badge for "Lost"/"Lost Resurrected" in Settings → Opportunity Stages reads light-error red (Badge `error` variant) instead of the neutral-grey above — the client wants Lost to visually read as a dead end on the management screen specifically. The Kanban board's Lost/Stalled column headers in the table above are unchanged and deliberately stay neutral, so a stalled deal doesn't read as alarming there. `STAGE_GROUP_BADGE_VARIANT` (shared with the Opportunity List table and Contact Detail's opportunity rail) is untouched; the settings screen applies a local override on top of it.
