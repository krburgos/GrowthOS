...
- Added `avatar_url text` (the `contact_avatar_storage` migration, Contact Detail redesign) — a per-contact profile picture, same shape as `accounts.logo_url`/`users.avatar_url` (§12).
- `companies` gained `linkedin_url text` (the `company_linkedin_url` migration) — the CRM company record's own LinkedIn page, distinct from `accounts.linkedin_url` (the MSP's own Company Profile LinkedIn, §6.1) and from `contacts.linkedin_url` (the contact's personal LinkedIn). Wired the same way as `companies.website`: editable on Contact Detail's Company card, set via Add Contact/Import/List Upload's company-matching flow, and a toggleable column on the Contacts table.
- Added `logo_url text` (the `crm_company_logo_storage` migration, Company Detail's Concept B redesign) — a fourth exception to §12's "no file attachments" line, alongside `accounts.logo_url`/`users.avatar_url`/`contacts.avatar_url`. Its own `crm-company-logos` storage bucket, deliberately separate from `company-logos` (the MSP's own branding logo) so the two are never confused. Editable from Company Detail's hero only — not exposed on Contact Detail or the Contacts table.
...
**Follow-up, same day — the From *address* itself:** Resend can send from any address on a domain verified in the account, not one fixed address, so the route tries the picked identity's (or, by default, the sender's own) real address as From first, and only falls back to the shared `EMAIL_FROM_ADDRESS` if Resend rejects it for an unverified domain — the real address is kept as Reply-To either way, since Reply-To isn't subject to domain authentication. In practice: an identity whose address is on a domain the account has verified in Resend sends from its own real address; an identity on a domain that can never be verified (a personal Gmail/Outlook address, or any domain not added to Resend) falls back to the shared address with the real one still reachable via Reply-To.
...
**Client-confirmed addition (2026-09-24)** — the Company Profile's Sales & Marketing list stops being  (a  of names) and becomes a real roster: **name, title, weekly hours commitment, and which of the 14 workstreams each person is responsible for**.

**Two rosters, one table.**  is  (the account's own staff) or  (a third party delivering a workstream, which may be CRO Leader or anyone else). The fields are identical, so a kind column beats two tables, and one workstream can carry both kinds at once.  is deliberately one free field so a row reads "Rosa Pineda - Northlight Events" as easily as a person alone.  is an optional link, since some staff are GrowthOS users and some are not.

**** joins a member to a  with its own weekly hours, unique per (member, step). The step list stays app code (), the same as .

**RLS** matches the Company Profile exactly, at the client's direction: read like ; write like  — Owner/Admin on their own account, or CRO Admin/Advisor anywhere. Note  has no partner clause, so neither do these: a partner reads an MSP's roster but does not edit it. Members are soft-deleted via  and have no delete policy; assignments are a join row rather than a record with history, so they do have one, the same shape  already uses.

**Deliberately not connected to the Command Center's hours (client-confirmed, "separate for now").** These are weekly intent;  records what was committed and achieved in a quarter. Nothing is enforced between a person's weekly commitment and the sum of their assignments either — the allocated figure is shown beside the commitment so it is visible, and that is all. **Two sources of truth for "outsourced" are also accepted knowingly:**  stays a manual per-workstream flag, and can disagree with whether an outsourced member is assigned to that step. The client chose to keep both; the Mission Card is where any disagreement shows.

Migration: , which also carries existing  values across as  members with blank title and hours. The column itself is left in place for now rather than dropped.
